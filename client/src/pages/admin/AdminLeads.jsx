...
const statusStyle = (status) => {
  ...
  if (status === 'Hot') return { bg: 'rgba(239,68,68,0.12)', color: '#ef4444', border: 'rgba(239,68,68,0.25)' };
...
const AdminLeads = () => {
  ...
  const hasFilters = filters.status || filters.city || filters.project || filters.assignedTo;
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface-0)', padding: '2rem 1.5rem' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            <select
              ...
              style={{
                appearance: 'none', background: 'var(--surface-1)', border: '1px solid var(--border)',
                borderRadius: 8, padding: '0.5rem 2rem 0.5rem 0.75rem', fontSize: '0.8rem',
                color: filters.assignedTo ? 'var(--text-primary)' : 'var(--text-muted)',
                outline: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)',
                transition: 'border-color 160ms ease-out',
              }}
              onFocus={e => e.target.style.borderColor = 'var(--accent)'}
          ...
          {hasFilters && (
            <button
              onClick={() => setFilters({ status: '', project: '', city: '', assignedTo: '' })}
              style={{
